...
const MyWeeklyCalendar = props => {
    ...
    const clickLastWeekHandler = () => {
        ...
        onWeekChange(lastWeekCurrDate.clone().weekday(props.startWeekday - 7))
        setCalendarReady(true)
    ...
    const clickNextWeekHandler = () => {
        ...
        onWeekChange(nextWeekCurrDate.clone().weekday(props.startWeekday - 7))
        setCalendarReady(true)
    }
    ...
    const onWeekChange = (weekdays) => {
        
        if (props.onWeekChange !== undefined) 
        props.onWeekChange(weekdays)

    }
    return (
        <View style={[styles.component, props.style]}>
            <View style={styles.header}>
                <TouchableOpacity style={{ padding: 10, backgroundColor: "blue", height: 60, justifyContent: "center" }} onPress={() => { clickLastWeekHandler(); }}>
                    <Text style={{ color: "white", fontSize: 11,fontWeight:"bold" }}>{'\u25C0'} Önceki Hafta</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity style={{ padding: 10, backgroundColor: "blue", height: 60, justifyContent: "center" }} onPress={()=>{clickNextWeekHandler();}}>
                    <Text style={{ color: "white", fontSize: 11 ,fontWeight:"bold" }}>Sonraki Hafta {'\u25B6'} </Text>
                </TouchableOpacity>
            </View>
            {/* <View style={styles.week}> */}
            {/* <View style={{justifyContent:"center",alignItems:"center",height:55,}}>
                <Text style={{fontWeight:"bold",color:"blue"}}>{ moment(new Date(currDate)).locale(props.locale).format("DD MMMM yyyy")} ve { moment(new Date(currDate)).add(6, 'days').locale(props.locale).format("DD MMMM yyyy")}  </Text>
                <Text style={{fontWeight:"bold",color:"blue"}}>Tarihleri arasındaki takvimi görüyorsun </Text>
...
MyWeeklyCalendar.defaultProps = { // All props are optional
    selected: moment(),
    startWeekday: 7,
